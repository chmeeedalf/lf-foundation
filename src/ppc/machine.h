...
#define FRAME_SET_ARGUMENT(FRAME_DATA, ARGUMENT_LOCATION, ARG_INFO) \
    do { if ((ARG_INFO).size < sizeof(void*)) \
...
#define FUNCTION_VALUE(TYPE, ARGS, RESULT_FRAME, RETURN_VALUE) \
    do {	int type_size = objc_sizeof_type(TYPE); \
	if(*(TYPE) == _C_FLT) { \
	    *(float*)(RETURN_VALUE) \
	    ...
	    *(double*)(RETURN_VALUE) \
	    ...
	    memcpy((RETURN_VALUE), \
		   ((char*)(RESULT_FRAME)) + sizeof(void*) - type_size, \
                   type_size); \
         else memcpy((RETURN_VALUE), (RESULT_FRAME), type_size); } while (0)
	...
	if(*(TYPE) == _C_FLT) \
		...
		*(double*)(RETURN_VALUE); \
	    ...
	    memcpy(((char*)(RESULT_FRAME)) + sizeof(void*) - type_size, \
                   (RETURN_VALUE), \
                   type_size); \
	else memcpy((RESULT_FRAME), (RETURN_VALUE), type_size); } while (0)
...
/* This macro determines the encoding of the next argument of a method. It is
   ...
   produce a String whose value represents the encoding and position of the
   current argument. STACKSIZE is a variable that counts the number of bytes
   occupied by the arguments on the stack. */

static inline int ROUND(int a, int b)
{
	return a * ((b + a - 1) / a);
}


static inline int __always_inline FUNCTION_ARG_ENCODING (CUMULATIVE_ARGS *CUM, const char *type, char *type_out, const int len_rem, int *STACK_ARGSIZE) 
{
	int offset = 0;
	bool reg = false;
	if (*type == _C_FLT || *type == _C_DBL) { 
		if (++CUM->float_args > 13) { 
			...
			CUM->stack_position += ROUND ((*CUM).stack_position, 
					__alignof__(double)); 
			offset = CUM->stack_position + OBJC_FORWARDING_MIN_OFFSET;
			...
			reg = true;
			offset = 40 + sizeof (double) * (CUM->float_args - 1); 
		...
		if (++CUM->int_args > 8) { 
			...
			offset = CUM->int_regs_position;
			reg = true;
			CUM->int_regs_position += ROUND (size, align); 
		} 
	} 
	return snprintf(type_out, len_rem, "%s%s%d", type,(reg?"+":""),offset);
}
